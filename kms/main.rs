...
fn initialize_logging() {
    ...
    tracing_subscriber::registry()
        // Send ALL logs to stderr so we can see errors on the serial console.
        .with(
            Layer::new()
...
async fn main() {
    ...
    tracing::info!("endorsements done");
    ...
    eprintln!("KeyManagementService Creating, connecting to {}", OAK_SESSION_SERVICE_ADDR);
    tracing::info!("KeyManagementService Creating");
    // Create the KeyManagementService.
    let channel = match transport::Endpoint::from_static(OAK_SESSION_SERVICE_ADDR)
        .connect()
        .await
    {
        Ok(ch) => {
            eprintln!("Successfully connected to storage service at {}", OAK_SESSION_SERVICE_ADDR);
            ch
        }
        Err(e) => {
            eprintln!("ERROR: failed to connect to storage service at {}: {:?}", OAK_SESSION_SERVICE_ADDR, e);
            panic!("failed to connect to storage service: {:?}", e);
        }
    };
    ...
    eprintln!("Starting gRPC server on [::]:8080");
    tracing::info!("Starting gRPC");
    // Start the gRPC server.
    orchestrator_client.notify_app_ready().await.expect("failed to notify that app is ready");
    eprintln!("notify_app_ready() done, server running");
    tracing::info!("notify_app_ready()");
