...
// The qemu and server modules are included as part of this crate via the mod.rs file
// which orchestrates the module hierarchy
...
pub const VM_LOCAL_PORT: u16 = 8080;

/// The local port that the Orchestrator should be listening on.
pub const VM_ORCHESTRATOR_LOCAL_PORT: u16 = 4000;

/// The fixed port for the data read/write service on the host.
/// Use a fixed port so QEMU guestfwd can be configured before the service starts.
pub const DATA_SERVICE_PORT: u16 = 9090;

/// The guest-accessible address for the data read/write service (via guestfwd).
/// This is the address the VM should connect to reach the host's data service.
pub const VM_DATA_SERVICE_ADDRESS: &str = "10.0.2.100:8081";
...
pub enum TestTypeArg {
    #[default]
    Normal,
    #[value(name = "policy-hash-mismatch")]
    PolicyHashMismatch,
    #[value(name = "access-policy-no-transforms")]
