...
pub struct Params {
    ...
    /// Host port where the data read/write service is running.
    /// When set, adds a guestfwd rule so the VM can reach the service at 10.0.2.100:8081.
    #[arg(long)]
...
impl Qemu {
    pub fn start(
        ...
        // Add guestfwd for data read/write service if port is specified
        // Guest can reach the service at 10.0.2.100:8081
        if let Some(data_service_port) = params.data_service_port {
            netdev_rules.push(format!(
                "guestfwd=tcp:10.0.2.100:8081-cmd:nc {host_address} {data_service_port}"
            ));
        }
