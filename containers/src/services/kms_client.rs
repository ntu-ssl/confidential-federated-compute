...
use kms_proto::fcp::confidentialcompute::{
    ...
    DeriveKeysRequest, GetKeysetRequest, RegisterPipelineInvocationRequest,
    RotateKeysetRequest, ReleaseResultsRequest,
};
use kms_proto::{
...
impl KmsClient {
    ...
    pub async fn derive_keys(
        ...
        debug!(
            "Deriving keys for keyset {} with {} policies",
            keyset_id,
            policy_hashes.len()
        );
        ...
        info!(
            "Derived {} public keys for keyset {}",
            response.public_keys.len(),
            keyset_id
        );
    ...
    pub async fn register_pipeline_invocation(
        ...
        debug!(
            "Registering pipeline invocation: {}",
        ...
        let request = RegisterPipelineInvocationRequest {
            ...
            intermediates_ttl: Some(ProtoDuration {
                seconds: intermediates_ttl_seconds,
                nanos: 0,
            }),
            keyset_ids,
        ...
        let keys_info: Vec<Vec<u8>> = response
            .keys
            .iter()
            .map(|k| format!("Key ID: {:?}", k.key_id).into_bytes())
            .collect();
    ...
    pub async fn authorize_transform(
        ...
        let final_evidence = evidence.ok_or_else(|| {
            anyhow!("Evidence must be provided for authorize_transform")
        })?;

        let final_endorsements = endorsements.ok_or_else(|| {
            anyhow!("Endorsements must be provided for authorize_transform")
        })?;
    ...
    pub async fn release_results(
        ...
        let releasable_results = release_tokens
            .into_iter()
            .map(|(release_token, signing_key_endorsement)| {
                ReleasableResult {
                    release_token,
